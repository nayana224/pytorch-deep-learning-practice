...
class XceptionBlock(nn.Module):
    ...
    def forward(self, x):
        y = self.body(x)
        if self.stride == 2:
            # 3x3 / stride 2 / padding 1 keeps the residual and main paths aligned
            # even for odd spatial sizes such as the 513x513 paper crop.
...
class ScaledXception65(nn.Module):
    ...
    def __init__(self, output_stride=16):
        ...
        self.middle = nn.Sequential(
            *[
                XceptionBlock(728, 728, dilation=middle_dilation, reps=3)
                for _ in range(4)
            ]
        )
...
class ASPP(nn.Module):
    def __init__(self, in_ch=2048, out_ch=256, output_stride=16):
        ...
        self.bs = nn.ModuleList(
            [SepConv(in_ch, out_ch, dilation=rate) for rate in rates]
        )
    ...
    def forward(self, x):
        size = x.shape[-2:]
        parts = [self.b0(x)] + [branch(x) for branch in self.bs]
